...
public class TopKIdentifierFlowId5{
				...
				for (String flowid : expectedHH){
					...
					for (FlowIdWithCount f : outputFlowBuckets){
						if (f.flowid.equals(flowid)){
							//System.err.println("FOUNDDDDDDDD");
							tempFlag = 1;
							break;
						}
					}
					if (tempFlag == 0){
						...
						//System.err.println("NEVER FOUND :(");
					}
				}

				for (int v = k[k_index]; v <= outputFlowBuckets.length; v += k[k_index]){
					numberOfFalseNegatives[k_index] = 0;
					numberOfFalsePositives[k_index] = 0;
					varianceFN[k_index] = 0;
					varianceFP[k_index] = 0;
					varianceRep[k_index] = 0;
					bigLoserPacketCount[k_index] = 0;
					cumDeviation[k_index] = 0;
					bigLoserPacketReported[k_index] = 0;
					observedHH = new HashMap<String, Long>();
					for (int i = 0; i < v && i < outputFlowBuckets.length; i++){
					...
					System.out.print("," + (double) missingFromTable[k_index]/numberOfTrials/expectedSize[k_index]);
					System.out.println();
				}
	...
	public static void main(String[] args){
		...
		//final int tableSize[] = {300, 600, 900, 1200, 1500 , 3000, 4500/*, 1800, 2100, 2400, 3000, 3600, 4200, 4500, 4800*/};
		final int tableSize[] = {300, 600, 900, 1200, 1500, 1800, 2400, 2700, 3000, 3600, 4200, 4800, 5100, 5400, 5700, 6000, 6300};
