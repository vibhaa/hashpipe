...
counter flow_packet_counter {
    type: packets;
    direct : update_hash_table;
}
...
action populate_hashtable() {
    modify_field_with_hash_based_offset(ingress_metadata.hash_index,
      0, ecmp_hash, HASH_TABLE_SIZE);
    register_read(ingress_metadata.flow_id, flow_ids,
      ingress_metadata.hash_index);
    // automatically updated
    //count(flow_packet_counter, ingress_metadata.hash_index);

    // flowlet_swtiching code
